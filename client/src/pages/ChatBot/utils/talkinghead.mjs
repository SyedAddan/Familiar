...
class TalkingHead {
    ...
    for (let [key, val] of Object.entries(p)) {
      ...
      if (ids[1] === "position" || ids[1] === "scale") {
        ...
        v = new THREE.Quaternion()
          .setFromEuler(new THREE.Euler(val.x, val.y, val.z, "XYZ"))
          .normalize();
  ...
  async showAvatar(avatar, onprogress = null) {
    ...
    this.setMood(
      this.moodName || this.avatar.avatarMood || this.opt.avatarMood
    );
    this.start();
  }
  ...
  animFactory(t, loop = false, scaleTime = 1, scaleValue = 1) {
    ...
    const delay = a.delay
      ? Array.isArray(a.delay)
        ? this.gaussianRandom(a.delay[0], a.delay[1], a.delay[2], a.delay[3])
        : a.delay
      : 0;
    if (a.hasOwnProperty("dt")) {
      a.dt.forEach((x, i) => {
        o.ts[i + 1] =
          o.ts[i] +
          (Array.isArray(x) ? this.gaussianRandom(x[0], x[1], x[2], x[3]) : x);
      });
    }
      ...
      const v = vs.map((x) => {
        if (x === null) {
          ...
          return (
            ...
            scaleValue * this.gaussianRandom(x[0], x[1], x[2], x[3])
          );
